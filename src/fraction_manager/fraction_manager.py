...
class FractionManager:
    ...
    def trace_solver(self, fract: Fraction, ta: complex):
        ...
        for i in range(self.root_iter):
            real_v = (
                trace_equation(fract, z + self.root_epsilon)
                - trace_equation(fract, z - self.root_epsilon)
            ) / (2 * self.root_epsilon)
            imag_v = (
                trace_equation(fract, z + self.root_epsilon * 1j)
                - trace_equation(fract, z - self.root_epsilon * 1j)
            ) / (2j * self.root_epsilon)
            deriv = (real_v + imag_v) / 2.0
        ...
        raise ValueError(
            f"Newton method failed after {self.root_iter} iterations ! Last values: f({z}) = {trace_eq_val}"
        )
